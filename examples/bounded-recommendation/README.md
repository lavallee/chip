...
|---|---|---|---|
| in | `finding` | `schemas/materiality-finding.json@1` | **exact same schema ref** as `publication-attention`'s `finding` output, so the circuit port match passes (§11). On a circuit the host delivers the finding as `activation["upstream"]`; in isolation (host `evaluate`) it rides inline in the signal's `content`. |
...
> the response's `body.recommendation` field.

## Effect

| Effect | Schema | Class | Approval |
|---|---|---|---|
| `recommend-research` | `schemas/research-recommendation.json@1` | `synthesize` | `human` |
...
`derive_effect_key(signal.lineageKey, "recommend-research", effect_target, promise_id)` —
`lineageKey` from the §8.1 signal envelope and `effect_target`/`promise_id`
host-injected into `config`, so the host's recomputation matches (§8.3).
...
|---|---|---|
| `positive` | positive | well-formed finding (inline in signal content) → one effect request, stable key |
| `quiet` | quiet | no upstream finding to act on → quiet, zero effects |
...
The fixtures deliver the finding inline in the signal's `content` so they are
host-`evaluate`-able without a circuit; per-lineage rate limiting
(`already-issued`) is state-dependent and covered by a direct unit test.

## How a host runs it

```python
run({"run_id": <host-minted run id>, "signal": <§8.1 signal envelope>,
     "upstream": <prior chip's finding response|None>, "state": <prior state|None>,
     "config": {"effect_target": "owner://research-ideas", "promise_id": <manifest id>},
     "gateway": <always-raises for a deterministic chip>})
# -> {"response": ..., "state": ..., "effects": [<effect request>|...], "stage_events": [...]}
```

Pure function of `(signal, upstream, state, config)`; no gateway call, no I/O, no
network, nothing imported beyond the standard library and `chip.*`. The response
is an §8.2 envelope with top-level `producedByChip`/`producedByRun` coordinates.
